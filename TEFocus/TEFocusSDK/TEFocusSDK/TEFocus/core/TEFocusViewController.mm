    ...
    if ([node.viewClassName isEqualToString:@"container"]) {
        ...
        if ([self getLocalImage:node]) {
            ...
            [imageView configureLayoutWithBlock:^(TAYGLayout * _Nonnull layout) {
                layout.isEnabled = YES;
                layout.position = TAYGPositionTypeAbsolute;
                // 设置背景图的大小为父视图的大小
                if ([self getHeiget:node] != 0) {
                    layout.height = TAYGPointValue([self getHeiget:node]);
                }
        ...
        if ([self getBackgroundURl:node]) {
            UIImageView *imageView = [[UIImageView alloc] init];
            UIActivityIndicatorView *activityIndicatorView = [[UIActivityIndicatorView alloc] init];
            if (@available(iOS 13.0, *)) {
                activityIndicatorView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleLarge; // 在iOS13及以上版本，使用UIActivityIndicatorViewStyleLarge样式
                activityIndicatorView.color = [UIColor systemGrayColor]; // 设置圆形进度指示器颜色为系统默认的灰色
            } else {
                activityIndicatorView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray; // 在iOS13以下版本，使用UIActivityIndicatorViewStyleGray样式
                activityIndicatorView.color = [UIColor systemGrayColor]; // 设置圆形进度指示器颜色为系统默认的灰色
            }
            activityIndicatorView.center = CGPointMake([self getWidth:node]/2, [self getHeiget:node]/2);
            [imageView addSubview:activityIndicatorView];
            [activityIndicatorView startAnimating];
            [imageView te_setImageWithURL:[NSURL URLWithString:[self getBackgroundURl:node]] placeholder:nil options:(TEWebImageOptions)TEWebImageOptionSetImageWithFadeAnimation completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, TEWebImageFromType from, TEWebImageStage stage, NSError * _Nullable error) {
        ...
        if ([self getImageURl:node]) {
            ...
            [imageView addSubview:activityIndicatorView];
            [activityIndicatorView startAnimating];
            [imageView te_setImageWithURL:[NSURL URLWithString:[self getImageURl:node]] placeholder:nil options:(TEWebImageOptions)TEWebImageOptionSetImageWithFadeAnimation completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, TEWebImageFromType from, TEWebImageStage stage, NSError * _Nullable error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [activityIndicatorView stopAnimating];
                });
            }];
            //[imageView te_setImageWithURL:[NSURL URLWithString:[self getImageURl:node]] options:TEWebImageOptionSetImageWithFadeAnimation];
        }
...
- (nullable NSString *)getBackgroundURl:(TEViewNode *)node {
    return node.layoutProperties[@"backgroundUrl"];
}
