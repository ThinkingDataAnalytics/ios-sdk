...
static BOOL TECGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, TECGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface _TEWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _TEWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_TEWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface TEWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) TEImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) TEWebImageProgressBlock progress;
@property (nonatomic, copy) TEWebImageTransformBlock transform;
@property (nonatomic, copy) TEWebImageCompletionBlock completion;
@end


@implementation TEWebImageOperation
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"TEWebImageOperation init error" reason:@"TEWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if (_taskID != UIBackgroundTaskInvalid) {
        [_TESharedApplication() endBackgroundTask:_taskID];
        _taskID = UIBackgroundTaskInvalid;
    }
    if ([self isExecuting]) {
        self.cancelled = YES;
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & TEWebImageOptionShowNetworkActivity)) {
                [TEWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, TEWebImageFromNone, TEWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & TEWebImageOptionUseNSURLCache) &&
            !(_options & TEWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:TEImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, TEWebImageFromMemoryCache, TEWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & TEWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:TEImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:TEImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & TEWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, TEWebImageFromNone, TEWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[_TEWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & TEWebImageOptionShowNetworkActivity)) {
                [TEWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, TEWebImageFromNone, TEWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, TEWebImageFromDiskCache, TEWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & TEWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([TEWebImageOperation _imageQueue], ^{
                        [_cache setImage:image imageData:data forKey:_cacheKey withType:TEImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            ...
            if (!image) {
                ...
                if (_options & TEWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, TEWebImageFromRemote, TEWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & TEWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & TEWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & TEWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & TEWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[TEImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == TEImageTypeUnknown ||
            _progressiveDecoder.type == TEImageTypeWebP ||
            _progressiveDecoder.type == TEImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != TEImageTypeJPEG &&
                _progressiveDecoder.type != TEImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            TEImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, TEWebImageFromRemote, TEWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == TEImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!TECGImageLastPixelFilled(image.CGImage)) return;
            ...
            image = [image te_imageByBlurRadius:radius tintColor:nil tintMode:0 saturation:1 maskImage:nil];
            
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, TEWebImageFromRemote, TEWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & TEWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & TEWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[TEImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image te_imageByDecoded];
                    if ([((TEImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    TEImageDecoder *decoder = [TEImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                TEImageType imageType = TEImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case TEImageTypeJPEG:
                    case TEImageTypeGIF:
                    case TEImageTypePNG:
                    case TEImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == TEImageTypeGIF ||
                                imageType == TEImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & TEWebImageOptionShowNetworkActivity)) {
                [TEWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, TEWebImageFromNone, TEWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & TEWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & TEWebImageOptionAllowBackgroundTask) && _TESharedApplication()) {
                    __weak __typeof__ (self) _self = self;
                    if (_taskID == UIBackgroundTaskInvalid) {
                        _taskID = [_TESharedApplication() beginBackgroundTaskWithExpirationHandler:^{
