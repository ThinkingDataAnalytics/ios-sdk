//
//  TEKVStorage.h
//  TECache <https://github.com/ibireme/TECache>
//
...
/**
 TEKVStorageItem is used by `TEKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface TEKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `TEKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use TEKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use TEKVStorageTypeFile to get better performance.
 * You can use TEKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, TEKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    TEKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    TEKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    TEKVStorageTypeMixed = 2,
};



/**
 TEKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for TEKVStorage is `initWithPath:type:`. 
 ...
 */
@interface TEKVStorage : NSObject
...
@property (nonatomic, readonly) TEKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(TEKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is TEKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is TEKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is TEKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(TEKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 TEKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is TEKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is TEKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is TEKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable TEKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable TEKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `TEKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TEKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `TEKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TEKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
