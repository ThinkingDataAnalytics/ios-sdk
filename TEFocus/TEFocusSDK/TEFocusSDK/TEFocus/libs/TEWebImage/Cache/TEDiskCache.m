...
static int64_t _TEDiskSpaceFree() {
    NSError *error = nil;
...
/// String's md5 hash.
static NSString *_TENSStringMD5(NSString *string) {
...
static void _TEDiskCacheInitGlobal() {
...
static TEDiskCache *_TEDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _TEDiskCacheInitGlobal();
...
static void _TEDiskCacheSetGlobal(TEDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _TEDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _TENSStringMD5(key);
    return filename;
}
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"TEDiskCache init error" reason:@"TEDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    TEDiskCache *globalCache = _TEDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    TEKVStorageType type;
    if (threshold == 0) {
        type = TEKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = TEKVStorageTypeSQLite;
    } else {
        type = TEKVStorageTypeMixed;
    }
    
    TEKVStorage *kv = [[TEKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    TEKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [TEDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [TEDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != TEKVStorageTypeSQLite) {
