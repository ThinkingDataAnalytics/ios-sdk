...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `TEAnimatedImage` protocol,
...
/**
 The TEAnimatedImage protocol declares the required methods for animated image
 display with TEAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to TEAnimatedImageView.image or TEAnimatedImageView.highlightedImage
 to display animation.
 
 See `TEImage` and `TEFrameImage` for example.
 */
@protocol TEAnimatedImage <NSObject>
@required
