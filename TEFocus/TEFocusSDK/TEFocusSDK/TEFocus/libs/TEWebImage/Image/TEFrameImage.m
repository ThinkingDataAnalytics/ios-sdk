...
- (instancetype)initWithImagePaths:(NSArray *)paths frameDurations:(NSArray *)frameDurations loopCount:(NSUInteger)loopCount {
    ...
    UIImage *firstCG = [[[UIImage alloc] initWithData:firstData] te_imageByDecoded];
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    if (_imagePaths) {
        ...
        return [[UIImage imageWithData:data scale:scale] te_imageByDecoded];
    } else if (_imageDatas) {
        if (index >= _imageDatas.count) return nil;
        NSData *data = _imageDatas[index];
        return [[UIImage imageWithData:data scale:[UIScreen mainScreen].scale] te_imageByDecoded];
    } else {
