...
+ (TEImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (TEImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (TEImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        TEImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.te_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"TEImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"TEImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"TEImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"TEImageData"];
    } else {
        [super encodeWithCoder:aCoder]; // Apple use UIImagePNGRepresentation() to encode UIImage.
    }
}

#pragma mark - protocol TEAnimatedImage
