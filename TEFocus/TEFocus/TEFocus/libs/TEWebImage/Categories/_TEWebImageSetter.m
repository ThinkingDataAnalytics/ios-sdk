...
NSString *const _TEWebImageFadeAnimationKey = @"TEWebImageFade";
const NSTimeInterval _TEWebImageFadeTime = 0.2;
const NSTimeInterval _TEWebImageProgressiveFadeTime = 0.4;


@implementation _TEWebImageSetter {
...
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
    ...
    if (sentinel != _sentinel) {
        if (completion) completion(nil, imageURL, TEWebImageFromNone, TEWebImageStageCancelled, nil);
        return _sentinel;
    }
    ...
    if (!operation && completion) {
        NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : @"TEWebImageOperation create failed." };
        completion(nil, imageURL, TEWebImageFromNone, TEWebImageStageFinished, [NSError errorWithDomain:@"com.ibireme.webimage" code:-1 userInfo:userInfo]);
    }
