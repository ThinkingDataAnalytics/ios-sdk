...
@class TEMemoryCache, TEDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, TEImageCacheType) {
    /// No value.
    TEImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    TEImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    TEImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    TEImageCacheTypeAll    = TEImageCacheTypeMemory | TEImageCacheTypeDisk,
};


/**
 TEImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface TEImageCache : NSObject
...
/** The underlying memory cache. see `TEMemoryCache` for more information.*/
@property (strong, readonly) TEMemoryCache *memoryCache;

/** The underlying disk cache. see `TEDiskCache` for more information.*/
@property (strong, readonly) TEDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'TEImage' to decode 
...
/**
 ...
 @return  The singleton TEImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `TEImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `TEImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(TEImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `TEImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(TEImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(TEImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, TEImageCacheType type))block;
