...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, TEImageType) {
    TEImageTypeUnknown = 0, ///< unknown
    TEImageTypeJPEG,        ///< jpeg, jpg
    TEImageTypeJPEG2000,    ///< jp2
    TEImageTypeTIFF,        ///< tiff, tif
    TEImageTypeBMP,         ///< bmp
    TEImageTypeICO,         ///< ico
    TEImageTypeICNS,        ///< icns
    TEImageTypeGIF,         ///< gif
    TEImageTypePNG,         ///< png
    TEImageTypeWebP,        ///< webp
    TEImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, TEImageBlendOperation) {
 ...
 */
@interface TEImageFrame : NSObject <NSCopying>
...
@property (nonatomic) TEImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) TEImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    TEImageDecoder *decoder = [TEImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    TEImageDecoder *decoder = [[TEImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface TEImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) TEImageType type;          ///< Image data type.
 ...
 */
- (nullable TEImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in TEImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    TEImageEncoder *jpegEncoder = [[TEImageEncoder alloc] initWithType:TEImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    TEImageEncoder *gifEncoder = [[TEImageEncoder alloc] initWithType:TEImageTypeGIF];
 ...
 */
@interface TEImageEncoder : NSObject

@property (nonatomic, readonly) TEImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(TEImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(TEImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(TEImageDecoder *)decoder type:(TEImageType)type quality:(CGFloat)quality;
...
@interface UIImage (TEImageCoder)
 ...
 */
@property (nonatomic) BOOL te_isDecodedForDisplay;
 ...
 */
- (void)te_saveToAlbumWithCompletionBlock:(nullable void(^)(NSURL * _Nullable assetURL, NSError * _Nullable error))completionBlock;
 ...
 */
- (nullable NSData *)te_imageDataRepresentation;
...
CG_EXTERN TEImageType TEImageDetectType(CFDataRef data);

/// Convert TEImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable TEImageTypeToUTType(TEImageType type);

/// Convert UTI (such as kUTTypeJPEG) to TEImageType.
CG_EXTERN TEImageType TEImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable TEImageTypeGetExtension(TEImageType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef TECGColorSpaceGetDeviceRGB();

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef TECGColorSpaceGetDeviceGray();

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL TECGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL TECGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation TEUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger TEUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable TECGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable TECGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable TECGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable TECGImageCreateEncodedData(CGImageRef imageRef, TEImageType type, CGFloat quality);


/**
 Whether WebP is available in TEImage.
 */
CG_EXTERN BOOL TEImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger TEImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable TECGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, TEImagePreset) {
    TEImagePresetDefault = 0,  ///< default preset.
    TEImagePresetPicture,      ///< digital picture, like portrait, inner shot
    TEImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    TEImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    TEImagePresetIcon,         ///< small-sized colorful images
    TEImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is TEImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable TECGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             TEImagePreset preset);
