...
typedef NS_ENUM(NSUInteger, TEWebImageFromType) {
    ...
    TEWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    TEWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    TEWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    TEWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, TEWebImageStage) {
    
    /// Incomplete, progressive image.
    TEWebImageStageProgress  = -1,
    
    /// Cancelled.
    TEWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    TEWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^TEWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);

/**
 The block invoked before remote image fetch finished to do additional image process.
 
 @discussion This block will be invoked before `TEWebImageCompletionBlock` to give
 ...
 @example You can clip the image, blur it and add rounded corners with these code:
    ^(UIImage *image, NSURL *url) {
        ...
        image = [image te_imageByResizeToSize:CGSizeMake(100, 100) contentMode:UIViewContentModeScaleAspectFill];
        image = [image te_imageByBlurRadius:20 tintColor:nil tintMode:kCGBlendModeNormal saturation:1.2 maskImage:nil];
        image = [image te_imageByRoundCornerRadius:5];
        return image;
    }
 ...
 */
typedef UIImage * _Nullable (^TEWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^TEWebImageCompletionBlock)(UIImage * _Nullable image,
                                          NSURL *url,
                                          TEWebImageFromType from,
                                          TEWebImageStage stage,
 ...
 */
@interface TEWebImageManager : NSObject

/**
 Returns global TEWebImageManager instance.
 
 @return TEWebImageManager shared instance.
 */
 ...
 */
- (instancetype)initWithCache:(nullable TEImageCache *)cache
 ...
 */
- (nullable TEWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) TEImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) TEWebImageTransformBlock sharedTransformBlock;
...
/**
 ...
 @param url A specified URL
 @return Cache key used in TEImageCache.
 */
