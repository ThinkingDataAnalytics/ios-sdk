...
- (ThinkingAnalyticsSDK *)innerCreateLightInstance {
    ThinkingAnalyticsSDK *lightInstance = [[LightThinkingAnalyticsSDK alloc] initWithAPPID:self.config.appid withServerURL:self.config.configureURL withConfig:self.config];
...
- (instancetype)initWithConfig:(TDConfig *)config {
    if (self = [super init]) {
        ...
        [[TDAnalyticsAppGroupManager shareInstance] setReceiveUrl:self.config.configureURL appId:self.config.appid];
        ...
        TDLogInfo(@"initialized successfully!\n AppID: %@ \n ServerUrl: %@ \n Mode: %@ \n TimeZone: %@ \n DeviceID: %@ \n Lib: %@ \n LibVersion: %@", self.config.appid, self.config.configureURL, [self modeEnumToString:self.config.mode], self.config.defaultTimeZone, [TDAnalytics getDeviceId], [[TDDeviceInfo sharedManager] libName] ,[TDDeviceInfo libVersion]);
...
- (NSString *)description {
    return [NSString stringWithFormat:@"[ThinkingAnalyticsSDK] AppID: %@, ServerUrl: %@, Mode: %@, TimeZone: %@, DeviceID: %@, Lib: %@, LibVersion: %@", self.config.appid, self.config.configureURL, [self modeEnumToString:self.config.mode], self.config.defaultTimeZone, [TDAnalytics getDeviceId], [[TDDeviceInfo sharedManager] libName] ,[TDDeviceInfo libVersion]];
}
...
- (void)autoFlushWithTimer:(NSTimer *)timer {
    ...
    TDLogInfo(@"flush data by the timer.");
    [self.eventTracker flush];
}
