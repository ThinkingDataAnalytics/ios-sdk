...
- (instancetype)initLight:(NSString *)appid withServerURL:(NSString *)serverURL withConfig:(TDConfig *)config {
    if (self = [self init]) {
        ...
        self.eventTracker.thinkingAnalyticsInstance = self;
    }
    return self;
}
...
- (void)optOutTrackingAndDeleteUser {
    ...
    // 立即上报事件
    [self trackUserEvent:deleteEvent properties:nil];
...
- (void)trackUserEvent:(TAUserEvent *)event properties:(NSDictionary *)properties {
    ...
    // 发送数据
    [self.eventTracker track:jsonObj immediately:event.immediately];
}
...
- (void)trackEvent:(TATrackEvent *)event properties:(NSDictionary *)properties isH5:(BOOL)isH5 {
    ...
    if ([event isKindOfClass:[TAAutoTrackEvent class]]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSDictionary *autoDynamicSuperProperties = [self.autoTrackSuperProperty obtainDynamicSuperPropertiesWithType:autoEvent.autoTrackEventType currentProperties:jsonObj];
            autoEvent.autoDynamicSuperProperties = [TAPropertyValidator validateProperties:autoDynamicSuperProperties validator:autoEvent];
