...
+ (void)startAnalyticsWithConfig:(TDConfig *)config {
    ...
    config.configureURL = [config.configureURL ta_formatUrlString];
    NSString *url = config.configureURL;
    if (url.length == 0) {
        return;
    }
...
+ (NSString *)lightInstanceIdWithAppId:(NSString *)appId {
    ...
    if (instance) {
        ...
        return lightInstance.config.appid;
    }
    return nil;
}
...
+ (void)setSDKStatus:(TDTrackStatus)status {
    NSString *appId = [ThinkingAnalyticsSDK defaultAppId];
    [TDAnalytics setSDKStatus:status withAppId:appId];
}
