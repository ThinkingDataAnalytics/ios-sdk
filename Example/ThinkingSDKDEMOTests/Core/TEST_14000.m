...
//                NSDictionary *dic2 = [[ins2 valueForKey:@"dataQueue"] performSelector:@selector(getFirstRecords:withAppid:) withObject:@(TEST_MaxNumEvents*2) withObject:input2.appid];
                ...
                // 数量是否准确
                NSLog(@"dataSource1 %lu, contents1 %lu", self.dataSource1.count , contents1.count);
                NSLog(@"dataSource2 %lu, contents2 %lu", self.dataSource2.count , contents2.count);
                XCTAssertTrue(self.dataSource1.count == contents1.count);
                XCTAssertTrue(self.dataSource2.count == contents2.count);
